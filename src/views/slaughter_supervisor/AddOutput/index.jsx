...
import { getSlaughterTypes } from "../store";
...
function DynamicFields() {
  ...
  useEffect(() => {
    dispatch(getSlaughterTypes());
    console.log(store.slaughterTypes)
  }, [dispatch, store.slaughterTypes.length]);
  ...
  try {
      const res = await axios.post(
        "http://127.0.0.1:8000/slaughter-supervisor-api/add-output-slaughters",
        {
                    ...
                    <Controller
                      ...
                      render={({ field: { onChange, value } }) => (
                        <Select
                          ...
                          value={store.slaughterTypes.find((option) => option.value === value)}
                          options={store.slaughterTypes.map((option) => ({
                            value: option.id,
                            label: option.type,
                          }))}
                        />

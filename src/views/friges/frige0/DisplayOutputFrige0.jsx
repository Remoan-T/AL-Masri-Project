...
import { DisplayOutputZeroFrigeContent } from "../store";
...
const DataTablesBasic = () => {
    const columns = [
        ...
        {
            name: " نوع المادة",
            selector: (row) => row.zero_frige.warehouse.out_put__type__production.type,
        },
    ...
    const ExpandedComponent = ({ data }) => (
        <div>
            <DataTable
                ...
                data={store.OutputZeroFrigeContent}
                columns={columns2}
    ...
    useEffect(() => {
        dispatch(DisplayOutputZeroFrigeContent());
    }, [dispatch, store.OutputZeroFrigeContent.length]);
    ...
    const hasData = searchValue.length
        ? searchData.length > 0
        : store.OutputZeroFrigeContent.length > 0;
                        ...
                        <h3 className="text-success">
                            ...
                            {store.OutputZeroFrigeContent == ""
                                ...
                                : `عدد المخرجات : ${searchValue.length
                                    ? searchData.length
                                    : store.OutputZeroFrigeContent.length
                                }`}
                        </h3>
                    </h2>{" "}
                </CardTitle>
                ...
                <div className="d-flex mt-md-0 mt-1">
                    <UncontrolledButtonDropdown>
                        <DropdownToggle
                            ...
                            caret
                            outline
                            disabled={store.OutputZeroFrigeContent == ""}
                        >
                            <Share size={15} />
            ...
            {isLoading ? ( // Show the loading spinner while isLoading is true
                ...
                <div className="react-dataTable">
                    <DataTable
                        noHeader
                        pagination
                        data={searchValue.length ? searchData : store.OutputZeroFrigeContent}
                        columns={columns}
                        className="react-dataTable"

...
import { DisplayOutputfrige1content } from "../store";
...
const DataTablesBasic = () => {
    const columns = [
        ...
        {
            name: " نوع المادة",
            selector: (row) => row.detonator1.warehouse.out_put__type__production.type,
        },
    ...
    const ExpandedComponent = ({ data }) => (
        <div>
            <DataTable
                ...
                data={store.Outputfrige1content}
                columns={columns2}
    ...
    useEffect(() => {
        dispatch(DisplayOutputfrige1content());
    }, [dispatch, store.Outputfrige1content.length]);
    ...
    const hasData = searchValue.length
        ? searchData.length > 0
        : store.Outputfrige1content.length > 0;
                        ...
                        <h3 className="text-success">
                            ...
                            {store.Outputfrige1content == ""
                                ...
                                : `عدد المخرجات : ${searchValue.length
                                    ? searchData.length
                                    : store.Outputfrige1content.length
                                }`}
                        </h3>
                    </h2>{" "}
                </CardTitle>
                ...
                {/* <div className="d-flex mt-md-0 mt-1">
                    <UncontrolledButtonDropdown>
                        <DropdownToggle
                            ...
                            caret
                            outline
                            disabled={store.Outputfrige1content == ""}
                        >
                            <Share size={15} />
            ...
            {isLoading ? ( // Show the loading spinner while isLoading is true
                ...
                <div className="react-dataTable">
                    <DataTable
                        noHeader
                        pagination
                        data={searchValue.length ? searchData : store.Outputfrige1content}
                        columns={columns}
                        className="react-dataTable"

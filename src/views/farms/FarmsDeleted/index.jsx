...
const DataTablesBasic = () => {
    ...
    const handleExport = () => {
        ...
        const fileName ="المزارع المحذوفة";
        const formattedData = store.deletedFarms.map(
      ...
      const handleFilter = (e) => {
        ...
        if (value.length) {
          updatedData = store.deletedFarms.filter((item) => {
            const startsWith =
    ...
    const handleConfirmText = id => {
        return MySwal.fire({
          title: 'هل تريد استرجاع المزرعة ؟',
          text: "",
          icon: 'question',
          showCancelButton: true,
          confirmButtonText: 'استرجاع المزرعة',
    ...
    return (
        <Card className='overflow-hidden'>
            ...
            <CardTitle >
          {" "}
          <h2>المزارع المحذوفة<br/><br/><h3 className="text-danger">{store.deletedFarms == ""
                ? null
                : `عدد المزارع : ${
                    searchValue.length
                      ? filteredData.length
                      : store.deletedFarms.length
                  }`}</h3></h2>{" "}
        </CardTitle>
        <div className="d-flex mt-md-0 mt-1">
          <UncontrolledButtonDropdown>
            <DropdownToggle color="secondary" caret outline disabled={store.deletedFarms == ''}>
              <Share size={15} />
          ...
          <div className="spinner-border text-primary" role="status">
            ...
            <div className='react-dataTable'>
                <DataTable
                    noHeader
                    pagination
                    data={searchValue.length ? filteredData : store.deletedFarms}
                    columns={columns}

...
import { DisplayLakeContent, DisplayDropdawnZeroFrigeContent } from "../store";
...
const DataTablesBasic = () => {
    ...
    console.log("🚀 ~ file: index.jsx:133 ~ DataTablesBasic ~ store.LakeContent:", store.LakeContent)
    useEffect(() => {
        dispatch(DisplayDropdawnZeroFrigeContent());
    }, [dispatch, store.DropdawnZeroFrigeContent.length]);
    console.log("🚀 ~ file: index.jsx:142 ~ DataTablesBasic ~ store.DropdawnZeroFrigeContent:", store.DropdawnZeroFrigeContent)
    ...
    const SingleOrder = store.LakeContent.find((statement) => statement.id === Number(id));
    console.log(SingleOrder)
    if (!SingleOrder) {
        return null; // or return a loading indicator or error message
    }

    const tableData = [SingleOrder.InputLakeContent];
    ...
    const handleButtonClicked = () => {
        ...
        axios
            .post("http://127.0.0.1:8000/warehouse-supervisor-api/set-from-lake-to-output", requestBody)
            .then((response) => {
            ...
            <Row className="justify-content-end mx-0">
                <InputGroup className="mb-2">
                    <Select
                        ...
                        // isDisabled={!check}
                        options={store.DropdawnZeroFrigeContent.map((option) => ({
                            value: option.to,
                            label: option.to,
                        }))}

...
import { getOutputManufacturingTypes } from "../store";
...
function DynamicFields() {
  ...
  useEffect(() => {
    dispatch(getOutputManufacturingTypes());
    console.log(store.outputManufacturingTypes)
  }, [dispatch, store.outputManufacturingTypes.length]);
  ...
  try {
      const res = await axios.post(
        `http://127.0.0.1:8000/manufacturing-supervisor-api/add-output-munufacturing/${id}`,
        {
                    ...
                    <Controller
                      ...
                      render={({ field: { onChange, value } }) => (
                        <Select
                          ...
                          value={store.outputManufacturingTypes.find((option) => option.value === value)}
                          options={store.outputManufacturingTypes.map((option) => ({
                            value: option.id,
                            label: option.type,
                          }))}
                        />

...
import { getMunufacturingOutput } from "../store";
...
const DataTablesBasic = () => {
  ...
  const handleExport = () => {
    ...
    const formattedData = store.munufacturingOutput.map(
      ({
        request_type,
        farm,
        selling_port,
  ...
  const handleFilter = (e) => {
    ...
    if (value.length) {
      updatedData = store.munufacturingOutput.filter((item) => {
  ...
  const ExpandedComponent = ({ data }) => (
    <div>
      <DataTable
        ...
        data={data.detail_output_manufacturing}
        columns={columns2}
  ...
  useEffect(() => {
    dispatch(getMunufacturingOutput());
  }, [dispatch, store.munufacturingOutput.length]);
  ...
  const hasData = searchValue.length
    ? searchData.length > 0
    : store.munufacturingOutput.length > 0;
            ...
            <h3 className="text-success">
              ...
              {store.munufacturingOutput == ""
                ...
                : `عدد المخرجات : ${
                    searchValue.length
                      ? searchData.length
                      : store.munufacturingOutput.length
                  }`}
            </h3>
          </h2>{" "}
        </CardTitle>
        ...
        {/* <div className="d-flex mt-md-0 mt-1">
          <UncontrolledButtonDropdown>
            <DropdownToggle
              ...
              caret
              outline
              disabled={store.munufacturingOutput == ""}
            >
              <Share size={15} />
      ...
      {isLoading ? ( // Show the loading spinner while isLoading is true
        ...
        <div className="react-dataTable">
          <DataTable
            noHeader
            pagination
            data={searchValue.length ? searchData : store.munufacturingOutput}
            columns={columns}
            className="react-dataTable"
            expandableRows={true}

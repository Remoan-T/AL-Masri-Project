...
import { getMunufacturingInput } from "../store";
...
const DataTablesBasic = () => {
  ...
  useEffect(() => {
    dispatch(getMunufacturingInput());
  }, [dispatch, store.munufacturingInput.length]);
  ...
  const handleExport = () => {
    ...
    const fileName = "مدخلات قسم التصنيع";
    const formattedData = store.munufacturingInput.map(
  ...
  const handleFilter = (e) => {
    ...
    if (value.length) {
      updatedData = store.munufacturingInput.filter((item) => {
        const startsWith =
  ...
  const hasData = searchValue.length
    ...
    : store.munufacturingInput.length > 0;
            ...
            <h3 className="text-success">
              {store.munufacturingInput == ""
                ...
                : `عدد المدخلات : ${
                    searchValue.length
                      ? filteredData.length
                      : store.munufacturingInput.length
                  }`}
            </h3>
          </h2>{" "}
        </CardTitle>
        <div className="d-flex mt-md-0 mt-1">
          <UncontrolledButtonDropdown>
            <DropdownToggle
              ...
              caret
              outline
              disabled={store.munufacturingInput == ""}
            >
              <Share size={15} />
      ...
      {isLoading ? ( // Show the loading spinner while isLoading is true
        ...
        <div className="react-dataTable">
          <DataTable
            noHeader
            pagination
            data={searchValue.length ? filteredData : store.munufacturingInput}
            columns={columns}

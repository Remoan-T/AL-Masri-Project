...
import { getTotalManufacturingInput } from "../store";
...
const DataTablesBasic = () => {
    ...
    },
    {
        ...
        cell: row => (
            ...
            <div>
                ...
                <a href={`/manufacturing/AddOutput/${row.type_id}`}>اضافة خرج</a>
            </div>
        )
    }
  ...
  useEffect(() => {
    dispatch(getTotalManufacturingInput());
  }, [dispatch, store.totalManufacturingInput .length]);
  ...
  const handleExport = () => {
    ...
    const fileName = "كامل دخل التصنيع";
    const formattedData = store.totalManufacturingInput.map(
      ({ weight, type}) => ({
         النوع: type,
         الوزن: `${weight} كغ`,
  ...
  const handleFilter = (e) => {
    ...
    if (value.length) {
      updatedData = store.totalManufacturingInput.filter((item) => {
        const startsWith =
  ...
  const hasData = searchValue.length
    ...
    : store.totalManufacturingInput .length > 0;
            ...
            <h3 className="text-success">
              {store.totalManufacturingInput  == ""
                ...
                : `عدد المدخلات : ${
                    searchValue.length
                      ? filteredData.length
                      : store.totalManufacturingInput .length
                  }`}
            </h3>
        ...
        <div className="d-flex mt-md-0 mt-1">
          <UncontrolledButtonDropdown>
            <DropdownToggle
              ...
              caret
              outline
              disabled={store.totalManufacturingInput  == ""}
            >
              <Share size={15} />
      ...
      {isLoading ? ( // Show the loading spinner while isLoading is true
        ...
        <div className="react-dataTable">
          <DataTable
            noHeader
            pagination
            data={searchValue.length ? filteredData : store.totalManufacturingInput }
            columns={columns}

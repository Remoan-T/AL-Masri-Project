...
import { getDeletedDrivers, RestoreDriver } from '../store'
...
const DataTablesBasic = () => {
    ...
    }, [dispatch, store.deletedDrivers.length])
    ...
    const handleConfirmText = (id) => {
        return MySwal.fire({
          title: "هل تريد استرجاع السائق ؟",
          text: "",
          icon: "question",
          showCancelButton: true,
          confirmButtonText: "استرجاع السائق",
      ...
      const handleFilter = (e) => {
        ...
        if (value.length) {
          updatedData = store.deletedDrivers.filter((item) => {
            const startsWith =
      ...
      const handleExport = () => {
        ...
        const fileName = "السائقين المحذوفين";
        const formattedData = store.deletedDrivers.map(
    ...
    return (
        <Card className='overflow-hidden'>
            ...
            <CardTitle >
          {" "}
          <h2>السائقين المحذوفين<br/><br/><h3 className="text-success">{store.deletedDrivers == ""
                ? null
                : `عدد السائقين المحذوفين : ${
                    searchValue.length
                      ? filteredData.length
                      : store.deletedDrivers.length
                  }`}</h3></h2>{" "}
        </CardTitle>
                <div className="d-flex mt-md-0 mt-1">
          <UncontrolledButtonDropdown>
            <DropdownToggle color="secondary" caret outline disabled={store.deletedDrivers == ''}> 
              <Share size={15} />
          ...
          <div className="spinner-border text-primary" role="status">
            ...
            <div className='react-dataTable'>
                <DataTable
                    noHeader
                    pagination
                    data={searchValue.length ? filteredData : store.deletedDrivers}
                    columns={columns}
